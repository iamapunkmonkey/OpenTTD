...
public:
		...
		switch (widget) {
			...
			case WID_TT_BUTTONS:
				for (uint i = WID_TT_BEGIN; i < WID_TT_END; i++) {
					if (i == WID_TT_LOADING || i == WID_TT_TRACKGRASS) continue; // Do not draw button for invisible loading indicators.
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		if (widget >= WID_TT_BEGIN && widget < WID_TT_END) {
			...
			if (i == WID_TT_LOADING || i == WID_TT_TRACKGRASS || i == WID_TT_END) return;
...
static const NWidgetPart _nested_transparency_widgets[] = {
	...
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_IMGBTN, COLOUR_DARK_GREEN, WID_TT_TRACKGRASS), SetMinimalSize(22, 22), SetFill(0, 1), SetDataTip(SPR_IMG_RAIL_NS, STR_TRANSPARENT_TRACKGRASS_TOOLTIP),
